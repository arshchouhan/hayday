...
const TREATMENTS = [
    { label: 'Group Movement',    key: 'group-movement'    },
    { label: 'Location Movement', key: 'location-movement' },
];

const FloatingInput = ({ label, required, type = 'text', value, onChange }) => (
    <div className="flex flex-col gap-1">
        <label className="text-[11px] font-semibold text-[#7C3AED]">
            {label}{required && <span className="ml-0.5 text-red-400">*</span>}
        </label>
        <div className="flex items-center rounded-md border border-[#D1D5DB] bg-white px-3 py-2 focus-within:border-[#7C3AED] focus-within:ring-1 focus-within:ring-[#7C3AED]">
...
const FloatingSelect = ({ label, required, placeholder, options = [], value, onChange }) => (
    <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center rounded-md border border-[#D1D5DB] bg-white px-3 py-2 focus-within:border-[#7C3AED]">
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-5 rounded-2xl border-2 border-dashed border-gray-200 bg-white/70 px-8 py-16 text-center">
            <svg viewBox="0 0 120 100" className="h-28 w-28 opacity-80" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="20" y="14" width="80" height="76" rx="8" fill="#F3EEFF" stroke="#7C3AED" strokeWidth="2.5"/>
                <rect x="42" y="8" width="36" height="14" rx="5" fill="white" stroke="#7C3AED" strokeWidth="2"/>
                {/* arrow right */}
                <path d="M40 52 H80 M68 40 L80 52 L68 64" stroke="#7C3AED" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.6"/>
                <circle cx="97" cy="24" r="4" fill="#7C3AED" opacity="0.3"/>
                <circle cx="108" cy="14" r="2.5" fill="#7C3AED" opacity="0.2"/>
                <circle cx="104" cy="32" r="2" fill="#7C3AED" opacity="0.15"/>
            </svg>
            <div>
                <p className="text-[15px] font-bold text-[#1a1a2e]">Select a movement type to get started</p>
                <p className="mt-1 text-[13px] text-gray-400">Choose a movement type above and the form will appear here</p>
            </div>
        </div>
    );
}
...
function PageLoader() {
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-[#F4F6F8]">
            ...
            <p className="text-[14px] font-semibold text-[#7C3AED]">Loading animal data…</p>
        </div>
    );
}
...
export default function MovementActivity() {
    ...
    const handleTreatmentChange = (e) => {
        ...
        timerRef.current = setTimeout(() => {
            ...
            navigate(`/farm/activity/movement/${animalId}/${key}?date=${activityDate}`, { replace: false });
        }, 750);
    };
            ...
            <div className="flex h-full w-full flex-col overflow-hidden bg-[#F4F6F8]">
                <div className="shrink-0 border-b border-gray-200 bg-white px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <button onClick={handleBack} className="flex items-center gap-1.5 text-[14px] font-bold text-[#7C3AED] hover:opacity-75 transition">
                            ...
                            {animal && (
                                <div className="mt-1 flex items-center gap-2">
                                    <div className={`h-3 w-3 rounded-full ${animal.ear_tag_color || 'bg-purple-500'}`} />
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    <div className="rounded-xl border border-gray-200 bg-white p-6">
                        <p className="mb-4 text-[11px] font-semibold uppercase tracking-widest text-gray-400">
                            Step 1 — Select Movement Type
                        </p>
                        <div className="grid grid-cols-3 gap-4">
                            <FloatingInput
                                label="Activity Date" required type="date"
                                value={activityDate}
                                onChange={e => { setActivityDate(e.target.value); setIsDirty(true); }}
                            />
                            <FloatingSelect
                                label="Movement Type" required
