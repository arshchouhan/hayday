...
══════════════════════════════════════════════════════════════════ */
const TREATMENTS = [
    { label: 'Breeding',         key: 'breeding'         },
    { label: 'Calving',          key: 'calving'          },
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-5 rounded-2xl border-2 border-dashed border-gray-200 bg-white/70 px-8 py-16 text-center">
            <svg viewBox="0 0 120 100" className="h-28 w-28 opacity-80" fill="none" xmlns="http://www.w3.org/2000/svg">
                {/* heart / breeding icon */}
                <rect x="20" y="14" width="80" height="76" rx="8" fill="#EEF6FF" stroke="#3B82F6" strokeWidth="2.5"/>
                <rect x="42" y="8" width="36" height="14" rx="5" fill="white" stroke="#3B82F6" strokeWidth="2"/>
                <path d="M60 65 C55 60 38 50 38 38 C38 30 46 24 60 36 C74 24 82 30 82 38 C82 50 65 60 60 65Z"
                    fill="#3B82F6" opacity="0.2" stroke="#3B82F6" strokeWidth="2" strokeLinejoin="round"/>
                <circle cx="97" cy="24" r="4" fill="#3B82F6" opacity="0.3"/>
                <circle cx="108" cy="14" r="2.5" fill="#3B82F6" opacity="0.2"/>
                <circle cx="104" cy="32" r="2" fill="#3B82F6" opacity="0.15"/>
            </svg>
            <div>
                <p className="text-[15px] font-bold text-[#1a1a2e]">Select an activity to get started</p>
                <p className="mt-1 text-[13px] text-gray-400">Choose a breeding activity above and the form will appear here</p>
            </div>
        </div>
    );
}
...
/* ══════════════════════════════════════════════════════════════════
   SKELETON  (shown as loader after selection)
══════════════════════════════════════════════════════════════════ */
...
function PageLoader() {
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-[#F4F6F8]">
            ...
            <p className="text-[14px] font-semibold text-[#3B82F6]">Loading animal data…</p>
        </div>
    );
}
            ...
            <div className="modal-card relative z-10 mx-4 w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div className="flex flex-col items-center gap-3 px-7 pb-7 pt-6 text-center">
                    ...
                    <p className="text-[13px] leading-relaxed text-gray-500">
                        You have a selection in progress. Going back will <strong className="text-gray-700">discard your progress</strong>.
                    </p>
...
/* ══════════════════════════════════════════════════════════════════
   MAIN  —  BreedingActivity  (selector)
══════════════════════════════════════════════════════════════════ */
export default function BreedingActivity() {
    ...
    /* ── Treatment selection → brief skeleton loader → navigate ───── */
    const handleTreatmentChange = (e) => {
        ...
        timerRef.current = setTimeout(() => {
            ...
            navigate(
                `/farm/activity/breeding/${animalId}/${key}?date=${activityDate}`,
                { replace: false }
            );
        }, 750);
    };
        ...
        <>
            ...
            <div className="flex h-full w-full flex-col overflow-hidden bg-[#F4F6F8]">
                ...
                <div className="shrink-0 border-b border-gray-200 bg-white px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <button
                                onClick={handleBack}
                                className="flex items-center gap-1.5 text-[14px] font-bold text-[#3B82F6] hover:opacity-75 transition"
                            >
                            ...
                            {animal && (
                                <div className="mt-1 flex items-center gap-2">
                                    <div className={`h-3 w-3 rounded-full ${animal.ear_tag_color || 'bg-blue-500'}`} />
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    ...
                    {/* Step 2: skeleton loader OR idle illustration */}
                    {formLoading ? (
                        <>
