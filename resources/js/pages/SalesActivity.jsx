...
const TREATMENTS = [
    { label: 'Dead Animal Record', key: 'dead-animal'        },
    { label: 'Feeding Sale',       key: 'feeding-sale'       },
    { label: 'Weight Management',  key: 'weight-management'  },
];

const FloatingInput = ({ label, required, type = 'text', value, onChange }) => (
    <div className="flex flex-col gap-1">
        <label className="text-[11px] font-semibold text-[#DC2626]">
            {label}{required && <span className="ml-0.5 text-red-400">*</span>}
        </label>
        <div className="flex items-center rounded-md border border-[#D1D5DB] bg-white px-3 py-2 focus-within:border-[#DC2626] focus-within:ring-1 focus-within:ring-[#DC2626]">
...
const FloatingSelect = ({ label, required, placeholder, options = [], value, onChange }) => (
    <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center rounded-md border border-[#D1D5DB] bg-white px-3 py-2 focus-within:border-[#DC2626]">
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-5 rounded-2xl border-2 border-dashed border-gray-200 bg-white/70 px-8 py-16 text-center">
            <svg viewBox="0 0 120 100" className="h-28 w-28 opacity-80" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="20" y="14" width="80" height="76" rx="8" fill="#FEF2F2" stroke="#DC2626" strokeWidth="2.5"/>
                <rect x="42" y="8" width="36" height="14" rx="5" fill="white" stroke="#DC2626" strokeWidth="2"/>
                {/* tag icon */}
                <path d="M44 38 L58 38 L76 52 L58 66 L44 66 Z" stroke="#DC2626" strokeWidth="2" fill="#DC2626" opacity="0.15" strokeLinejoin="round"/>
                <circle cx="51" cy="52" r="3" fill="#DC2626" opacity="0.5"/>
                <circle cx="97" cy="24" r="4" fill="#DC2626" opacity="0.3"/>
                <circle cx="108" cy="14" r="2.5" fill="#DC2626" opacity="0.2"/>
                <circle cx="104" cy="32" r="2" fill="#DC2626" opacity="0.15"/>
            </svg>
            <div>
                <p className="text-[15px] font-bold text-[#1a1a2e]">Select a record type to get started</p>
                <p className="mt-1 text-[13px] text-gray-400">Choose a record type above and the form will appear here</p>
            </div>
        </div>
    );
}
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-[#F4F6F8]">
            <div className="relative h-14 w-14">
                ...
                <div className="absolute inset-0 animate-spin rounded-full border-4 border-transparent border-t-[#DC2626]" />
            </div>
            <p className="text-[14px] font-semibold text-[#DC2626]">Loading animal data…</p>
        </div>
    );
}
            ...
            <div className="modal-card relative z-10 mx-4 w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div className="flex flex-col items-center gap-3 px-7 pb-7 pt-6 text-center">
                    ...
                    <p className="text-[13px] leading-relaxed text-gray-500">
                        Going back will <strong className="text-gray-700">discard your progress</strong>.
                    </p>
...
export default function SalesActivity() {
    ...
    const handleTreatmentChange = (e) => {
        ...
        timerRef.current = setTimeout(() => {
            ...
            navigate(`/farm/activity/sales/${animalId}/${key}?date=${activityDate}`, { replace: false });
        }, 750);
    };
    ...
    const handleDiscard = () => { clearTimeout(timerRef.current); setShowWarning(false); navigate(-1); };
    ...
    return (
        <>
            <style>{STYLES}</style>
            {showWarning && <UnsavedWarningModal onDiscard={handleDiscard} onKeepEditing={() => setShowWarning(false)} />}

            <div className="flex h-full w-full flex-col overflow-hidden bg-[#F4F6F8]">
                <div className="shrink-0 border-b border-gray-200 bg-white px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <button onClick={handleBack} className="flex items-center gap-1.5 text-[14px] font-bold text-[#DC2626] hover:opacity-75 transition">
                        ...
                        <button onClick={handleBack} className="rounded-full border border-gray-300 bg-white px-6 py-2 text-[13px] font-semibold text-gray-600 hover:bg-gray-50 transition">
                            Cancel
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    <div className="rounded-xl border border-gray-200 bg-white p-6">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <FloatingInput
                                label="Record Date" required type="date"
                                value={activityDate}
                                onChange={e => { setActivityDate(e.target.value); setIsDirty(true); }}
                            />
                            <FloatingSelect
                                label="Record Type" required
